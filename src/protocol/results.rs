...
impl IntervalResult for UdpReceiveResult {
    ...
    fn to_string(&self, bit: bool) -> String {
        ...
        if self.jitter_seconds.is_some() {
            output.push_str(&format!(
                "\njitter: {:.12}s over {} consecutive packets",
        ...
        output.push_str(&format!(
            ...
            self.latency_avg_seconds, self.latency_max_seconds, self.latency_min_seconds
        ));
        output
    }
}
...
impl StreamResults for UdpStreamResults {
    ...
    fn to_json(&self, omit_seconds: usize) -> serde_json::Value {
        ...
        //let sum : Vec<f64> = self.receive_results.iter().map(|x| x.latency.iter()).flatten().cloned().collect();
        //let count = sum.iter().count();
        //let sum = sum.iter().sum::<f64>();
